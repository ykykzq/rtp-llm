...
void addTensorList(c10::impl::GenericDict&                          payload,
    ...
    for (size_t i = 0; i < tensors->size(); ++i) {
        if ((*tensors)[i].defined() && c10::isFloat8Type((*tensors)[i].scalar_type())) {
            float8_dtypes.insert(std::string(name) + "[" + std::to_string(i) + "]",
                                 c10::toString((*tensors)[i].scalar_type()));
        }
    ...
    ~Worker() {
        {
            ...
            shutdown_deadline_ = std::chrono::steady_clock::now() + std::chrono::microseconds(kEventWaitTimeoutUs);
            stopping_          = true;
        }
