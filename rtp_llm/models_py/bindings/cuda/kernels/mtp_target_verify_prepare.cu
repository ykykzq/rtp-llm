...
__global__ void mtpRowLogSumExpKernel(const scalar_t* __restrict__ logits,
                                      float* __restrict__ row_logsumexp,
                                      int64_t row_stride,
    ...
    if (threadIdx.x == 0) {
        row_logsumexp[row] = logf(reduction[0]) + row_max;
    }
}

template<typename scalar_t>
void launchMtpRowLogSumExp(const torch::Tensor& logits,
                           torch::Tensor&       row_logsumexp,
                           int64_t              real_vocab_size,
                           cudaStream_t         stream) {
    constexpr int block_size = 256;
    mtpRowLogSumExpKernel<scalar_t><<<static_cast<unsigned int>(logits.size(0)), block_size, 0, stream>>>(
        logits.data_ptr<scalar_t>(), row_logsumexp.data_ptr<float>(), logits.stride(0), real_vocab_size);
}
...
void invokeMtpRowLogSumExp(const torch::Tensor& logits,
    ...
    RTP_LLM_CHECK_WITH_INFO(real_vocab_size > 0 && real_vocab_size <= logits.size(1),
                            ...
                            real_vocab_size,
                            logits.size(1));
    RTP_LLM_CHECK_WITH_INFO(row_logsumexp.defined() && row_logsumexp.is_cuda(),
                            "MTP row_logsumexp must be a CUDA tensor");
    RTP_LLM_CHECK_WITH_INFO(row_logsumexp.scalar_type() == torch::kFloat32, "MTP row_logsumexp must be float32");
    RTP_LLM_CHECK_WITH_INFO(row_logsumexp.is_contiguous() && row_logsumexp.numel() == logits.size(0),
                            "MTP row_logsumexp must be contiguous with one value per logits row");
    if (logits.size(0) == 0) {
        return;
    }
    ...
    switch (logits.scalar_type()) {
        case torch::kFloat32:
            launchMtpRowLogSumExp<float>(logits, row_logsumexp, real_vocab_size, stream);
            break;
        case torch::kFloat16:
            launchMtpRowLogSumExp<at::Half>(logits, row_logsumexp, real_vocab_size, stream);
            break;
        case torch::kBFloat16:
            launchMtpRowLogSumExp<at::BFloat16>(logits, row_logsumexp, real_vocab_size, stream);
            break;
        default:
...
#pragma unroll
    ...
    if (row_has_nan) {
        ...
        row_logsumexp = logf(reduction_values[0]) + row_max;
    }
    ...
    if (tid == 0) {
        ...
        token_logprobs[selected_row] = static_cast<float>(row_logits[emitted_token_id]) - row_logsumexp;
    }
    ...
    if constexpr (LOCAL_TOP_K > 0) {
        ...
        for (int rank = 0; rank < top_k; ++rank) {
            ...
            if (tid == 0) {
                ...
                top_logprobs[output_offset]          = reduction_values[0] - row_logsumexp;
